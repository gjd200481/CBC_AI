...
def plot_physics_training(history, true_values, pred_values, figure_path=None, show=True):
    ...
    plt.xlabel("True phi")
    plt.ylabel("Pred phi")
    plt.title(f"Test pred vs true, error std={np.std(errors):.4f}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Train a physics-constrained CNN for two-beam phase inversion."
    )
    parser.add_argument(
        "--image-path",
        type=Path,
    ...
    parser.add_argument(
        "--model-path",
        type=Path,
        default=REPO_ROOT / "models" / "physics_cnn_lambda_0.1_main_clean.pth",
    )
    parser.add_argument(
        "--metrics-path",
        type=Path,
        default=REPO_ROOT / "result" / "metrics" / "physics_cnn_lambda_0.1_main_clean.csv",
    )
    parser.add_argument(
        "--summary-path",
        type=Path,
        default=REPO_ROOT / "result" / "metrics" / "physics_cnn_lambda_0.1_main_clean_summary.csv",
    )
    parser.add_argument(
        "--figure-path",
        type=Path,
        default=REPO_ROOT / "result" / "figures" / "physics_cnn_lambda_0.1_main_clean.png",
    )
    ...
    optics_model = TwoBeamFourierOptics(
    ...
    torch.save(
        {
            ...
            "physics_model": "TwoBeamFourierOptics",
        },
        args.model_path,
    )
