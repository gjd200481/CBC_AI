...
def generate_images_from_fixed_phases(
    ...
    """用固定 6 路相位数组生成指定噪声强度下的 7 光束远场图像。"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate seven-beam noise robustness datasets with shared phase samples."
    )
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("dataset/seven_beam/noise_robustness"))
    ...
    for noise_sigma in args.noise_levels:
        ...
        noise_seed = args.seed + int(round(noise_sigma * 10000)) + 71
        ...
        config["dataset_family"] = "seven_beam_noise_robustness_shared_phases"
        ...
        print(
            ...
            f"phases={phases.shape}, noise_sigma={noise_sigma}, noise_seed={noise_seed}"
        )
